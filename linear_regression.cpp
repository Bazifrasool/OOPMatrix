...
template <typename T> Vector1D<T> linear_regression<T>::getWeights() {
  return weights;
}
template <typename T> T linear_regression<T>::getBias() { return bias; }
...
template <typename T>
Vector1D<T> linear_regression<T>::predict(Matrix<T> const &X) const {
  // To be completed
