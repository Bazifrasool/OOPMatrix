...
template <typename T> Vector1D<T> Vector1D<T>::operator+(Vector1D<T> &obj) {
...
template <typename T> T &Vector1D<T>::operator[](int index) {
  if (index < __vector.size()) {
    return __vector[index];
  }
  return __vector[index];
}
template <typename T> int Vector1D<T>::size() { return __vector.size(); }
...
template <typename T> Vector1D<T> &Matrix<T>::operator[](int row) {
  if (row < this->rows) {
...
template <typename T> Vector1D<T> &Matrix<T>::dot_product(Vector1D<T> vec) {
